...
const formatDate = (dateString) => {
  ...
  const parts = dateString.split('-'); // Splitting yyyy-mm-dd
  if (parts.length === 3) {
    return `${parts[1]}/${parts[2]}/${parts[0]}`; // Reordering to mm/dd/yyyy
  }
  return dateString;
};
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 border border-gray-200 rounded-lg p-4 bg-gray-50/50">
        ...
        {/* FIX: Corrected typo in the name attribute below */}
...
export default function Leads() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const adminUsers = response.data.filter(emp => emp.role === 'Admin');
  ...
  const handleSaveNew = async (e) => {
    ...
    try {
      const payload = { ...formData, Date_Added: new Date().toISOString().split('T')[0] };
      await axios.post('http://localhost:8000/api/leads', payload);
      fetchLeads();
      setIsAddOpen(false);
  ...
  const confirmDelete = async () => {
    try {
      ...
      setErrorMsg("Failed to delete the lead.");
    }
  };
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedLeads(currentLeads.map(lead => lead.Lead_ID));
    } else {
      setSelectedLeads([]);
    }
  };
  ...
  const handleSelectOne = (e, leadId) => {
    if (e.target.checked) setSelectedLeads(prev => [...prev, leadId]);
  ...
  const getSelectedAdminName = () => {
    ...
    const admin = admins.find(a => {
      const aId = a.id || a.User_ID || a.Employee_ID;
      return aId.toString() === selectedAdminId.toString();
    });
      ...
      <main className="pt-28 px-8 pb-12 max-w-[96%] mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {!isViewer && (
              <>
                <button className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-5 py-2 rounded-md font-medium transition-colors shadow-sm text-sm">
                  Import
                </button>
                <button 
                  onClick={() => { setFormData(emptyForm); setIsAddOpen(true); setErrorMsg(''); }} 
        ...
        {errorMsg && errorMsg.includes('connect') && (
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm text-left relative">
            <thead className="bg-[#7E3A99] text-[#f8f7fa] font-semibold select-none">
              <tr>
                ...
                {selectedLeads.length > 0 && !isViewer ? (
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex items-center gap-4 border-l border-white/20 pl-6 relative">
                        ...
                        <div className="relative">
                          ...
                          {isAssignOpen && (
                            <div className="absolute top-full left-0 mt-3 w-64 bg-white rounded-xl shadow-xl border border-gray-100 z-[100] overflow-visible text-gray-800">
                              <div className="p-4">
                                ...
                                <div className="relative mb-4">
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setIsEmployeeSelectOpen(!isEmployeeSelectOpen);
                                    }}
                                    className={`w-full flex items-center justify-between border ${isEmployeeSelectOpen ? 'border-[#7E3A99] ring-2 ring-[#7E3A99]/20' : 'border-gray-300 hover:border-gray-400'} rounded-lg p-2 text-sm bg-white transition-all cursor-pointer outline-none relative z-[110]`}
                                  >
                                    <span className={`block truncate ${selectedAdminId ? 'text-gray-800 font-medium' : 'text-gray-400'}`}>
                                      {getSelectedAdminName()}
                                    </span>
                                    <svg xmlns="http://www.w3.org/2000/svg" className={`w-4 h-4 text-gray-500 transition-transform duration-200 flex-shrink-0 ${isEmployeeSelectOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                                  ...
                                  {isEmployeeSelectOpen && (
                                    <>
                                      <div className="fixed inset-0 z-[120]" onClick={(e) => { e.stopPropagation(); setIsEmployeeSelectOpen(false); }}></div>
                                      <div className="absolute left-0 top-full mt-1.5 w-full bg-white rounded-xl shadow-lg border border-gray-100 z-[130] py-1.5 max-h-48 overflow-y-auto animate-fade-in-down">
                                        {admins.length === 0 ? (
                                          ...
                                          admins.map(admin => {
                                            ...
                                            return (
                                              <div
                                                ...
                                                className={`px-4 py-2.5 text-sm font-medium cursor-pointer transition-colors relative z-[135] ${
                                                  selectedAdminId === adminId ? 'bg-purple-50 text-[#7E3A99]' : 'text-gray-700 hover:bg-gray-50'
                                                }`}
                                              >
                                                {admin.name}
                                              </div>
                                ...
                                <div className="flex justify-end gap-2 relative z-[105]">
                                  <button 
                                    onClick={() => {
                                      setIsAssignOpen(false);
                                      setIsEmployeeSelectOpen(false);
                                    }}
                                    className="px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100 rounded-md font-medium transition-colors"
                                  >
                                    Cancel
                                  </button>
                                  <button 
                                    ...
                                    className="px-3 py-1.5 text-xs bg-[#7E3A99] hover:bg-[#19a828] text-white rounded-md font-medium transition-colors disabled:opacity-50"
                                  >
                                    Assign
                                  </button>
                                </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {currentLeads.map((lead) => {
                ...
                return (
                  <tr key={lead.Lead_ID} className={`transition-colors ${isSelected ? 'bg-purple-50 hover:bg-purple-100' : 'odd:bg-white even:bg-gray-50 hover:bg-gray-100'}`}>
                    ...
                    <td className="px-6 py-4 text-gray-600 whitespace-nowrap">{formatDate(lead.Date_Added)}</td>
              ...
              {currentLeads.length === 0 && (
                <tr><td colSpan={isViewer ? "8" : "9"} className="px-6 py-8 text-center text-gray-500">No leads found.</td></tr>
              )}
            </tbody>
          </table>
