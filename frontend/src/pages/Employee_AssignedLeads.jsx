...
const getChipColor = (value) => {
  switch (value) {
    ...
    case 'Contacted': return 'bg-[#fdf2f8] text-[#be185d] hover:bg-[#fce7f3]'; // Pinkish
  ...
  return (
    <div className="relative inline-block text-left w-full max-w-[120px] mx-auto">
      ...
      >
        ...
        <svg width="7" height="5" viewBox="0 0 7 5" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="opacity-70 flex-shrink-0 mt-[1px]">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 min-w-[130px] bg-white rounded-lg shadow-[0_2px_10px_rgba(0,0,0,0.15)] border border-gray-100 z-50 py-1 overflow-hidden animate-fade-in-down">
          {options.map((opt) => (
            <div
              ...
              className={`px-3 py-1.5 text-[12px] font-medium cursor-pointer transition-colors flex items-center gap-2 ${
                value === opt ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${getChipColor(opt).split(' ')[0]}`}></div>
...
export default function Employee_AssignedLeads() {
  ...
  const fetchAssignedLeads = async () => {
    ...
    if (!userId) {
      console.error("Could not find the User ID in Local Storage!");
      return;
    }
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm text-left">
              <thead className="bg-[#7E3A99] text-white select-none">
                <tr>
                  ...
                  <th className="px-4 py-4 rounded-tr-lg"></th> {/* Action header removed */}
                </tr>
              </thead>
                  ...
                  <tr key={lead.Lead_ID} className="hover:bg-purple-50/30 transition-colors group align-middle">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap sticky left-0 bg-white group-hover:bg-[#faf5fc] transition-colors shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] z-10">
                      <button 
                        ...
                        className="flex flex-col text-left group/btn outline-none"
                      >
                    ...
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        ...
                        isOpen={activeDropdown === `${lead.Lead_ID}-Lead_Status`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Lead_Status`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Lead_Status', val)} 
                      />
                    </td>
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        value={lead.Inquiry_Sent} 
                        options={OPTIONS.Inquiry_Sent} 
                        isOpen={activeDropdown === `${lead.Lead_ID}-Inquiry_Sent`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Inquiry_Sent`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Inquiry_Sent', val)} 
                      />
                    </td>
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        ...
                        isOpen={activeDropdown === `${lead.Lead_ID}-Inquiry_Type`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Inquiry_Type`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Inquiry_Type', val)} 
                      />
                    </td>
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        value={lead.Replied} 
                        options={OPTIONS.Replied} 
                        isOpen={activeDropdown === `${lead.Lead_ID}-Replied`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Replied`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Replied', val)} 
                      />
                    </td>
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        value={lead.Email_Sent} 
                        options={OPTIONS.Email_Sent} 
                        isOpen={activeDropdown === `${lead.Lead_ID}-Email_Sent`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Email_Sent`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Email_Sent', val)} 
                      />
                    </td>
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        value={lead.Email_Replied} 
                        options={OPTIONS.Email_Replied} 
                        isOpen={activeDropdown === `${lead.Lead_ID}-Email_Replied`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Email_Replied`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Email_Replied', val)} 
                      />
                    </td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <ChipSelect 
                        value={lead.Meeting_Booked} 
                        options={OPTIONS.Meeting_Booked} 
                        isOpen={activeDropdown === `${lead.Lead_ID}-Meeting_Booked`}
                        onToggle={() => toggleDropdown(`${lead.Lead_ID}-Meeting_Booked`)}
                        onChange={(val) => handleDropdownChange(lead.Lead_ID, 'Meeting_Booked', val)} 
                      />
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button 
                        onClick={() => handleAppointmentClick(lead.Lead_ID)}
                        className="p-1.5 text-[#7E3A99] hover:text-[#19a828] hover:bg-purple-50 rounded-md transition-colors"
                        title="Schedule Appointment"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.8" stroke="currentColor" className="w-5 h-5 mx-auto">
                ...
                {currentLeads.length === 0 && (
                  <tr>
                    <td colSpan="11" className="px-6 py-12 text-center text-gray-500">
                      You have no leads assigned to your pipeline yet.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-down">
            ...
            <div className="px-6 py-4 border-t bg-gray-50 flex justify-end items-center">
              <button onClick={() => setIsViewOpen(false)} className="px-6 py-2 bg-[#7E3A99] hover:bg-[#19a828] text-white rounded-md font-medium transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
